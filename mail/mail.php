...
$config = array(
    ...
    'use_ssl'  => '1',
    'port'     => '465',
    'account'  => '2274976814@qq.com',
    'name'     => '王睿个人主页留言',
    'password' => 'emuifckbgbotebhf',
);

// return html 或者 json 或者 empty
$return = 'html';
...
//接收post和get
$subject = "您有新的留言内容，来自：" . $config['name'];
$messages = array("来自IP：" . $_SERVER['REMOTE_ADDR'] . " 的留言内容：");
...
if(count($messages) > 1) {
    //追加空行和时间记录
    array_unshift($messages, "", "时间 : " . date('Y-m-d H:i:s'));
    //同时记录一份内容到文件
    file_put_contents('email.txt', implode("\r\n", $messages));

    $message = implode("<br/>", $messages);
    $result = send($subject, $message);

    if ($return == 'json') {
        //json 返回
        if ($result) {
            die(json_encode(array("code" => 1, "msg" => "提交成功")));
        } else {
            die(json_encode(array("code" => -1, "msg" => "提交失败")));
        }
    } else if ($return == 'html') {
        echo '<!DOCTYPE html>
        <html lang="zh-CN">
        ...
        <body>';
        if ($result) {
            echo '<center>提交成功</center>';
        } else {
            echo '<center>提交失败</center>';
        }

        echo '<script type="text/javascript">setTimeout(function(){window.location.go(-1);}, 3000);</script>';
        echo '</body>';
        die();
    }
} else {
...
function send($subject, $message) {
    ...
    $mail->addReplyTo($config['account'], $config['name']);
    //Set who the message is to be sent to
    $mail->addAddress($config['account'], "");
